...
/**
 * Converts a String to a {@link java.time.LocalDate}.
...
public class LocalDateConverter extends AbstractDateTimeConverter<LocalDate> {
    ...
    @Override
    public LocalDate convert(String value, Annotation[] annotations, Activity activity) throws Exception {
        if (value == null) {
            return null;
        }
        String format = findFormat(annotations);
        StringifyContext stringifyContext = activity.getStringifyContext();
        return stringifyContext.toLocalDate(value, format);
    }
