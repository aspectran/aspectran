 ...
 * <p>A token is the fundamental unit of an AsEL expression, representing either a literal
 * text segment or a dynamic placeholder that is resolved at runtime. Dynamic tokens are
 * enclosed in special symbols and curly braces (e.g., {@code #{...}}), allowing access
 * to various data within the application context.
 *
 * <h3>Token Expression Format</h3>
 * A token expression follows the general format:
 * <br>{@code token_symbol{directive:token_name^getter_name:default_value}}
 *
 * <h3>Supported Token Types</h3>
 * The following symbols distinguish the providers of values:
 * <dl>
 *   <dt>{@code #} - Bean</dt>
 *   <dd>Refers to a bean from the Bean Registry.
 *     <ul>
 *       <li>{@code #{beanId}} - Retrieves a bean by its ID.</li>
 *       <li>{@code #{beanId^propertyName}} - Accesses a property of the retrieved bean.</li>
 *       <li>{@code #{class:className}} - Retrieves a bean by its class type.</li>
 *       <li>{@code #{class:className^staticPropertyName}} - Accesses a static property of a class.</li>
 *       <li>{@code #{field:className^staticFieldName}} - Accesses a static field of a class.</li>
 *     </ul>
 *   </dd>
 *   <dt>{@code @} - Attribute</dt>
 *   <dd>Refers to an attribute in the current {@code ActivityContext}.
 *     <ul>
 *       <li>{@code @{attributeName}} - Retrieves an attribute by name.</li>
 *       <li>{@code @{attributeName^propertyName}} - Accesses a property of the attribute object.</li>
 *       <li>{@code @{attributeName:defaultValue}} - Provides a default value if the attribute is null.</li>
 *     </ul>
 *   </dd>
 *   <dt>{@code $} - Parameter</dt>
 *   <dd>Refers to a request parameter.
 *     <ul>
 *       <li>{@code ${parameterName}} - Retrieves a request parameter by name.</li>
 *       <li>{@code ${parameterName:defaultValue}} - Provides a default value if the parameter is not present.</li>
 *     </ul>
 *   </dd>
 *   <dt>{@code %} - Property</dt>
 *   <dd>Refers to a property from the application's environment.
 *     <ul>
 *       <li>{@code %{propertyName}} - Retrieves a property by name.</li>
 *       <li>{@code %{system:java.version}} - Retrieves a Java system property.</li>
 *       <li>{@code %{classpath:path/to/file.properties^propName}} - Loads properties from a classpath resource
 *           and retrieves a specific property.</li>
 *     </ul>
 *   </dd>
 *   <dt>{@code ~} - Template</dt>
 *   <dd>Renders a template and includes its output.
 *     <ul>
 *       <li>{@code ~{templateId}} - Renders the template specified by the ID.</li>
 *       <li>{@code ~{templateId:defaultContent}} - Provides default content if the template rendering results in null.</li>
 *     </ul>
 *   </dd>
 * </dl>
 *
...
public class Token implements BeanReferenceable, Replicable<Token> {
    ...
    /**
     * Converts this token back into its original AsEL string representation.
     * <p>This method reconstructs the expression string (e.g., {@code ${name:defaultValue}})
     * from the token's properties, effectively reversing the parsing process. For a {@link TokenType#TEXT}
     * token, it returns the raw text content.</p>
     ...
     * @return a new array containing replicated tokens, or {@code null} if the input is {@code null}
     */
    public static Token[] replicate(Token[] tokens) {
        if (tokens == null) {
            return null;
        }
    ...
    /**
     * Checks if the given expression string likely contains any AsEL tokens.
     * <p>This method performs a quick scan for token-like patterns (e.g., {@code #{...}}, {@code ${...}})
     * and is used as an optimization to avoid the overhead of full parsing if no tokens are present.</p>
     * @param expression the string to check
     * @return {@code true} if the string appears to contain tokens; {@code false} otherwise
     */
    ...
    /**
     * Resolves and sets the {@code valueProvider} for a bean token by loading the specified
     * class, field, or method.
     * <p>This is a pre-processing step to optimize evaluation by caching reflection lookups
     * (e.g., {@link Class}, {@link Field}, {@link Method} objects). When a token is evaluated,
     * this cached provider is used directly, avoiding repeated and costly reflection operations.</p>
     * @param token the bean token to resolve; must be of type {@link TokenType#BEAN}
     * @param classLoader the class loader to use for loading classes
     * @throws InvalidTokenException if a required element like a field or method name is missing
     * @throws IllegalArgumentException if the specified class, field, or method cannot be found
     */
