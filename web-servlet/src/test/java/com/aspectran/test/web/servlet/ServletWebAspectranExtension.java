 ...
 */
public class ServletWebAspectranExtension extends AspectranExtension {

    @Override
    public void beforeAll(@NonNull ExtensionContext context) throws Exception {
        Class<?> testClass = context.getRequiredTestClass();
        ServletWebAspectranTest annotation = testClass.getAnnotation(ServletWebAspectranTest.class);
        if (annotation != null) {
    ...
    protected void bootstrap(@NonNull ExtensionContext context, @NonNull AspectranTest annotation, @NonNull AspectranConfig aspectranConfig)
        ...
        DefaultServletWebService webService = DefaultServletWebServiceBuilder.build(servletContext);
        webService.start();

        context.getStore(NAMESPACE).put(DefaultServletWebService.class, webService);
    ...
    public void afterAll(@NonNull ExtensionContext context) throws Exception {
        DefaultServletWebService webService = context.getStore(NAMESPACE).get(DefaultServletWebService.class, DefaultServletWebService.class);
        if (webService != null) {
            webService.stop();
        }
    }
    ...
    public boolean supportsParameter(@NonNull ParameterContext parameterContext, @NonNull ExtensionContext extensionContext)
        ...
        return super.supportsParameter(parameterContext, extensionContext) ||
                type == WebService.class || type == DefaultServletWebService.class ||
                type == ServletWebActivityTester.class;
    }
    ...
    public Object resolveParameter(@NonNull ParameterContext parameterContext, @NonNull ExtensionContext extensionContext)
            throws ParameterResolutionException {
        Class<?> type = parameterContext.getParameter().getType();
        DefaultServletWebService webService = extensionContext.getStore(NAMESPACE).get(DefaultServletWebService.class, DefaultServletWebService.class);
        if (webService != null) {
            if (type == WebService.class || type == DefaultServletWebService.class) {
                return webService;
            } else if (type == ServletWebActivityTester.class) {
                return new ServletWebActivityTester(webService);
            }
        }
    ...
    @NonNull
    private AspectranTest adapt(@NonNull final ServletWebAspectranTest web) {
