 ...
 * the specific world of {@link HttpServletRequest},
 * {@link HttpServletResponse}, and {@link ServletContext}.
 * </p>
 *
 * @since 2.0.0
 */
public interface ServletWebService extends WebService {

    /**
     * ServletContext attribute name used to obtain the root WebService object.
     */
    String ROOT_WEB_SERVICE_ATTR_NAME = ServletWebService.class.getName() + ".ROOT";

    /**
     * ServletRequest attribute name used to obtain the derived WebService object.
     */
    String DERIVED_WEB_SERVICE_ATTR_NAME = ServletWebService.class.getName() + ".DERIVED";
    ...
    /**
     * Binds a {@link WebService} instance to a {@link ServletWebActivity}'s request attributes.
     * This is typically used for derived services within a specific request scope.
     * @param activity the web activity to bind to
     * @param service the web service instance to bind
     */
    static void bind(ServletWebActivity activity, WebService service) {
    ...
    /**
     * Finds the root {@link DefaultServletWebService} from the {@link ServletContext}.
     ...
     */
    @NonNull
    static DefaultServletWebService findWebService(ServletContext servletContext) {
        Assert.notNull(servletContext, "servletContext must not be null");
        DefaultServletWebService webService = findWebService(servletContext, ROOT_WEB_SERVICE_ATTR_NAME);
        if (webService == null) {
    ...
    /**
     * Finds the {@link DefaultServletWebService} associated with the given {@link ServletRequest}.
     ...
     */
    @NonNull
    static DefaultServletWebService findWebService(ServletRequest servletRequest) {
        Assert.notNull(servletRequest, "servletRequest must not be null");
        DefaultServletWebService webService = findWebService(servletRequest, DERIVED_WEB_SERVICE_ATTR_NAME);
        if (webService == null) {
    ...
    /**
     * Internal helper method to find a {@link DefaultServletWebService} from a {@link ServletContext} attribute.
     * @param servletContext the servlet context to search within
     * @param attrName the name of the {@link ServletContext} attribute to look for
     * @return the found {@link DefaultServletWebService}, or {@code null} if not found or not of the correct type
     */
    @Nullable
    private static DefaultServletWebService findWebService(@NonNull ServletContext servletContext, String attrName) {
        Object attr = servletContext.getAttribute(attrName);
        if (attr == null) {
            return null;
        }
        if (!(attr instanceof DefaultServletWebService webService)) {
            throw new IllegalStateException("Context attribute [" + attr + "] is not of type [" +
                    DefaultServletWebService.class.getName() + "]");
        }
        return webService;
    }

    /**
     * Internal helper method to find a {@link DefaultServletWebService} from a {@link ServletRequest} attribute.
     * @param servletRequest the servlet request to search within
     * @param attrName the name of the {@link ServletRequest} attribute to look for
     * @return the found {@link DefaultServletWebService}, or {@code null} if not found or not of the correct type
     */
    @Nullable
    private static DefaultServletWebService findWebService(@NonNull ServletRequest servletRequest, String attrName) {
