...
public class PebbleViewDispatcher implements ViewDispatcher {
    ...
    public void dispatch(Activity activity, DispatchResponseRule dispatchResponseRule) throws ViewDispatchException {
        ...
        try {
            ...
            if (debugEnabled) {
                log.debug("Dispatch to Pebble template page [" + dispatchName + "]");
            }
        } catch (Exception e) {
            throw new ViewDispatchException("Failed to dispatch to Pebble template page " + dispatchResponseRule.toString(this, dispatchName), e);
        }
    }
